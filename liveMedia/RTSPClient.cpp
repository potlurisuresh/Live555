...
unsigned RTSPClient::sendYakAnnounceCommand(responseHandler* responseHandler, Authenticator* authenticator) {
    if (authenticator != NULL) fCurrentAuthenticator = *authenticator;
...
#if 0
      extraHeaders = (char*)"Content-Type: application/sdp\r\nVideo-payload: a=rtpmap:96 H264/90000|a=fmtp:96 profile-level-id=42001f;packetization-mode=1;sprop-parameter-sets=Z00AHqtAWh7TUBAQECA=,KO48MA==\r\nAudio-payload: a=rtpmap:99 AMR/8000/1|a=fmtp:99 octet-align=1\r\n";

        extraHeaders = (char*)"Content-Type: application/sdp\r\nVideo-payload: a=rtpmap:96 H264/90000|a=fmtp:96 profile-level-id=42001f;packetization-mode=1;sprop-parameter-sets=Z00AHqtAWh7TUBAQECA=,KO4Jww==\r\nAudio-payload: a=rtpmap:99 AMR/8000/1|a=fmtp:99 octet-align=1\r\n";
#else
        extraHeaders = (char*)"Content-Type: application/sdp\r\nVideo-payload: a=rtpmap:97 H264/90000|a=fmtp:97 profile-level-id=42001f;packetization-mode=1;sprop-parameter-sets=Z0IAHqtAoP01AQEBAg==,KM48MA==\r\nAudio-payload: a=rtpmap:99 AMR/8000/1|a=fmtp:99 octet-align=1\r\n";        
#endif
...
void RTSPClient::handleResponseBytes(int newBytesRead) {
  ...
  do {
    ...
    char const* audioPortParamsStr = NULL;
    char const* videoPortParamsStr = NULL;
    ...
    } else if (checkForHeader(lineStart, "Transport-video: server_port=", 29, audioPortParamsStr)) {
    } else if (checkForHeader(lineStart, "Transport-audio: server_port=", 29, videoPortParamsStr)) {
	...
	*responseEnd = saved;
	...
	if (responseCode == 200) {
	  ...
	  resultString = numBodyBytes > 0 ? strDup(bodyStart) : strDup(publicParamsStr);
        //if(strlen(audioPortParamsStr)!=0)
            resultString = strDup(audioPortParamsStr);
        //if(strlen(videoPortParamsStr)!=0)
            strcat(resultString, ",");
            strcat(resultString, videoPortParamsStr);
